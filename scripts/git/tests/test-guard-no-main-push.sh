...
# Run by hand with no push payload it must exit cleanly rather than block on a
# read that never receives input.
if timeout 10 bash "$GUARD" < /dev/null >/dev/null 2>&1; then
  printf '  PASS  %-38s (exit 0)\n' "hand-run without a push payload"
else
